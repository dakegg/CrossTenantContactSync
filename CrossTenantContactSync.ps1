...
function Merge-Config {
    ...
    $allKeys = @(
        ...
        'AppendSourceTenantToDisplayName','AllowUpnFallback','DisableDeletes'
    )
...
function Set-TargetMailContact {
    ...
    $syncKey = Get-SyncKey -SourceTenantId $Config.SourceTenantId -SourceObjectId $User.id
...
try {
    ...
    $deltaResult = Get-UserDeltaChanges -AccessToken $graphToken -DeltaLink $state.DeltaLink
    ...
    if ($TopUsers -eq 0 -and $deltaResult.DeltaLink) {
        $state.DeltaLink = $deltaResult.DeltaLink
        Save-State -Path $stateFile -State $state
        Write-Log "Saved new delta token."
    }
    else {
        Write-Log "No deltaLink returned; state not updated." 'WARN'
    }
